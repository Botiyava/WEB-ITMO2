...
function changeBorderButton() {
    document.querySelector(`#SetBorder`).value = `set ${document.getElementById(`chBB`).value}px ${document.getElementById(`chBS`).value}`;
}
function setBorder() {
    let tds = document.querySelectorAll(`#tableWrap td`);
    tds.forEach(td => {
        td.style = `border: ${document.getElementById(`chBB`).value}px ${document.getElementById(`chBS`).value}`;
    });
}
